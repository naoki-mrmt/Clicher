...
### 1.3 キャプチャHUD（Capture Mode）

`⌘⇧A` で起動するキャプチャモードHUD。Lark のスクショ機能のように、**1つのショートカットから全モードにアクセス**する設計。

#### ショートカット体系

| ショートカット | 動作 |
|--------------|------|
| **`⌘⇧A`** | **キャプチャHUDを起動**（メインショートカット。これだけ覚えればOK） |
| HUD内 `1` or クリック | エリアキャプチャ |
| HUD内 `2` or クリック | ウィンドウキャプチャ |
| HUD内 `3` or クリック | フルスクリーンキャプチャ |
| HUD内 `4` or クリック | スクロールキャプチャ（Phase 2） |
| HUD内 `5` or クリック | OCR テキスト認識（Phase 2） |
| HUD内 `6` or クリック | 画面録画（Phase 3） |
| `Esc` | HUDを閉じる |
...
| タスク | 詳細 |
|--------|------|
| HUDウィンドウ | `NSPanel` (.floating) 画面中央に表示。半透明背景 |
| モード選択 | アイコン + テキスト + 数字キーのグリッド。ホバーでハイライト |
| オプションバー | タイマー（なし/3秒/5秒/10秒）、背景設定、クロスヘア/ルーペのトグル |
| キーボード操作 | 数字キーで即モード選択 → 実行。Esc で閉じる |
| 状態記憶 | 最後に使ったモードをハイライト表示（次回起動時） |
| アニメーション | 表示/非表示にフェードイン・アウト |

#### 設計意図

- **`⌘⇧A` 1本で全機能にアクセス** — ユーザーが覚えるショートカットは1つだけ
- 個別ショートカット（`⌘⇧4` でエリア等）も設定画面でカスタム割り当て可能
- HUD は macOS 標準の `⌘⇧5` ツールバーに似た体験だが、より多機能
