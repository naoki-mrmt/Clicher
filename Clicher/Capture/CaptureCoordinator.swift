...
final class CaptureCoordinator {
    ...
    /// キャプチャ完了時のコールバック
    var onCaptureComplete: ((CaptureResult) -> Void)?

    private let captureService: ScreenCaptureServiceProtocol

    init(captureService: ScreenCaptureServiceProtocol = ScreenCaptureService()) {
    ...
    private func startWindowCapture() async {
        ...
        do {
            ...
            let image = try await captureService.captureWindow(selectedWindow)
