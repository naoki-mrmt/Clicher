...
enum AnnotateRenderer {
    /// 全アノテーション要素を描画
    static func render(items: [AnnotationItem], in context: CGContext, size: CGSize) {
        for item in items {
    ...
    /// 単一アノテーション要素を描画
    static func render(item: AnnotationItem, in context: CGContext, size: CGSize) {
        context.saveGState()
