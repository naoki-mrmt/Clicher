...
- Target macOS 14.0 (Sonoma) or later. This is a macOS-only project.
...
- SwiftUI backed up by `@Observable` classes for shared data. AppKit is used for low-level window management (NSPanel, NSStatusItem, global hotkeys).
- Do not introduce third-party frameworks without asking first.
- Use AppKit alongside SwiftUI where SwiftUI cannot fulfil macOS-specific requirements (menu bar, overlays, screen capture).
